...
There is a [BatchBuilder class](https://www.drupal.org/node/2875389) for batch API which provides a more streamlined object-oriented approach to creating batches.
...
Note. You can set up a `$batch` array with a title and a progress message with some variables that will get displayed.
...
Note. To execute the batch, the example shows a call to  `batch_process('node/1')`.  This could be any valid url alias e.g., `/admin/content`.
